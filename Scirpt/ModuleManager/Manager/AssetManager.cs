...
namespace GDG.ModuleManager
{
    ...
    public class AssetManager : LazySingleton<AssetManager>
    {
        ...
        #endregion
        #region 异步加载
        public void TryLoadAssetAsync<T>(string bundlename, string assetname, UnityAction<T> callback) where T : Object
        {
            MonoWorld.Instance.StartCoroutine(CoroutineTryLoadAssetAsync<T>(bundlename, assetname, callback));
        }
        public void LoadAssetAsync<T>(string bundlename, string assetname, UnityAction<T> callback) where T : Object
        {
            MonoWorld.Instance.StartCoroutine(CoroutineLoadAssetAsync<T>(bundlename, assetname, callback));
        }
        ...
        {
            MonoWorld.Instance.StartCoroutine(CoroutineLoadAssetAsync<T>(bundlename, assetname, path, mainABName, callback));
        }
        public void LoadAssetAsync(string bundlename, string assetname, System.Type type, UnityAction<Object> callback)
        {
            MonoWorld.Instance.StartCoroutine(CoroutineLoadAssetAsync(bundlename, assetname, type, callback));
        }
        ...
        {
            MonoWorld.Instance.StartCoroutine(CoroutineLoadAssetAsync(bundlename, assetname, path, mainABName, type, callback));
        }
