        ...
        static void SetDataToSheet<T>(T data, ExcelWorksheet sheet, int startline)
        {
            ...
            if (data is ICollection collection)
            {
                ...
                if (data is IDictionary dic)
                {
                    ...
                    foreach (var key in dic.Keys)
                    {
                        ...
                        {
                            if (!GlobalMethod.IsBaseType(item.GetValue(value).GetType()))
                            {
                ...
                else
                {
                    ...
                    if (!GlobalMethod.IsBaseType(type))
                    {
                        ...
                        foreach (var obj in collection)
                        {
                            ...
                            foreach (var item in vf)
                            {
                                if (!GlobalMethod.IsBaseType(item.GetValue(obj).GetType()))
                                {
            ...
            else
            {
                ...
                foreach (var field in fields)
                {
                    ...
                    if (!GlobalMethod.IsBaseType(field.GetValue(data).GetType()))
                    {
                ...
                foreach (var property in properties)
                {
                    ...
                    if (!GlobalMethod.IsBaseType(property.GetValue(data).GetType()))
                    {
        ...
        static T GetDataFromSheet<T>(ExcelWorksheet sheet, int startline, int rowCount) where T : new()
        {
            ...
            if (data is ICollection)
            {
                ...
                else
                {
                    ...
                    if (!GlobalMethod.IsBaseType(valueType))
                    {
                        isBaseType = false;
                        line += 1;
                    }
                ...
                if (!isBaseType)
                {
                    for (int i = startColumn; i <= sheet.Dimension.End.Column; i++)
                    {
                        if (field.Name == sheet.Cells[startline, i].Text)
                        {
                            ...
                            else if (GlobalMethod.IsBaseType(field.FieldType))
                ...
                if (!isBaseType)
                {
                    for (int i = startColumn; i <= sheet.Dimension.End.Column; i++)
                    {
                        if (property.Name == sheet.Cells[startline, i].Text)
                        {
                            ...
                            else if (GlobalMethod.IsBaseType(property.PropertyType))
