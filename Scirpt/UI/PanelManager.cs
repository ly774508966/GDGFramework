...
namespace GDG.UI
{
    public class PanelManager : LazySingleton<PanelManager>
    {
        ...
        {
            var obj = ResourcesManager.Instance.LoadResource<GameObject>(panelPath);
        ...
        {
            var obj = AssetManager.Instance.LoadAsset<GameObject>(bundlename,assetname,path,mainABName);
            obj.transform.SetParent(Canvas.transform);
            if(obj.TryGetComponent<BasePanel>(out BasePanel panel))
            {
                RegisterPanel(obj.name,panel);
