...
namespace GDG.Async
{
    ...
    public class AsyncWebRequest
    {
        public static DownloadInfo DownloadTexture2D(string url, Action<Texture2D> callback, Action<UnityWebRequest> requestCallback = null)
        {
            ...
            MonoWorld.Instance.StartCoroutine(AsyncWaitDownloadFinish<Texture2D>(downloadInfo, url, callback, default(AudioType), null, requestCallback));
            return downloadInfo;
        }
        ...
        {
            var downloadInfo = new DownloadInfo();
            MonoWorld.Instance.StartCoroutine(AsyncWaitDownloadFinish<AssetBundle>(downloadInfo, url, callback, default(AudioType), null, requestCallback));
            return downloadInfo;
        }
        public static DownloadInfo DownloadData(string url, Action<byte[]> callback, Action<UnityWebRequest> requestCallback = null)
        {
            var downloadInfo = new DownloadInfo();
            MonoWorld.Instance.StartCoroutine(AsyncWaitDownloadFinish<byte[]>(downloadInfo, url, callback, default(AudioType), null, requestCallback));
            return downloadInfo;
        }
        ...
        {
            var downloadInfo = new DownloadInfo();
            MonoWorld.Instance.StartCoroutine(AsyncWaitDownloadFinish<AudioClip>(downloadInfo, url, callback, audioType, null, requestCallback));
            return downloadInfo;
        }
        public static DownloadInfo DownloadFile(string url, string filePath, Action<UnityWebRequest> callback = null)
        {
            var downloadInfo = new DownloadInfo();
            MonoWorld.Instance.StartCoroutine(AsyncWaitDownloadFinish<FileInfo>(downloadInfo, url, null, default(AudioType), filePath, callback));
            return downloadInfo;
        }
        public static UploadInfo Post(string url, string postData, Action<UnityWebRequest> requestCallback = null)
        {
            ...
            MonoWorld.Instance.StartCoroutine(PostAsync(uploadInfo, url, requestCallback, postData));
            return uploadInfo;
        }
        public static UploadInfo Post(string url, WWWForm wWWForm, Action<UnityWebRequest> requestCallback = null)
        {
            var uploadInfo = new UploadInfo();
            MonoWorld.Instance.StartCoroutine(PostAsync(uploadInfo, url, requestCallback, null, wWWForm));
            return uploadInfo;
        }
        ...
        {
            var uploadInfo = new UploadInfo();
            MonoWorld.Instance.StartCoroutine(PostAsync(uploadInfo, url, requestCallback, null, null, multipartFormSections));
            return uploadInfo;
        }
        public static UploadInfo Post(string url, Dictionary<string, string> formFields, Action<UnityWebRequest> requestCallback = null)
        {
            var uploadInfo = new UploadInfo();
            MonoWorld.Instance.StartCoroutine(PostAsync(uploadInfo, url, requestCallback, null, null, null, formFields));
            return uploadInfo;
        }
        public static UploadInfo Put(string url, string bodyDataString ,Action<UnityWebRequest> requestCallback = null)
        {
            var uploadInfo = new UploadInfo();
            MonoWorld.Instance.StartCoroutine(PutAsync(uploadInfo, url, requestCallback, bodyDataString));
            return uploadInfo;
        }
        public static UploadInfo Put(string url,  byte[] bodyData ,Action<UnityWebRequest> requestCallback = null)
        {
            var uploadInfo = new UploadInfo();
            MonoWorld.Instance.StartCoroutine(PutAsync(uploadInfo, url, requestCallback, null , bodyData));
            return uploadInfo;
        }
