...
namespace GDG.Utils
{
    ...
    [System.Serializable]
    public abstract class SerializableDictionary<TKey, TValue> : SerializableDictionaryBase, IDictionary<TKey, TValue>, ISerializationCallbackReceiver, IDeserializationCallback
    {
        public TKey[] m_Keys;
        public TValue[] m_Values;
        ...
        public ICollection<TKey> Keys => m_Dic.Keys;
        public ICollection<TValue> Values => m_Dic.Values;
        public int Count => m_Dic.Count;
        public bool IsReadOnly => ((IDictionary<TKey, TValue>)m_Dic).IsReadOnly;
        public void Add(TKey key, TValue value) => m_Dic.Add(key, value);
        public void Add(KeyValuePair<TKey, TValue> item) => ((IDictionary<TKey, TValue>)m_Dic).Add(item);
        public void Clear() => m_Dic.Clear();
        public bool Contains(KeyValuePair<TKey, TValue> item) => ((IDictionary<TKey, TValue>)m_Dic).Contains(item);
        public bool ContainsKey(TKey key) => m_Dic.ContainsKey(key);
        public void CopyTo(KeyValuePair<TKey, TValue>[] array, int arrayIndex) => ((IDictionary<TKey, TValue>)m_Dic).CopyTo(array, arrayIndex);

        public IEnumerator<KeyValuePair<TKey, TValue>> GetEnumerator() => m_Dic.GetEnumerator();
        public bool Remove(TKey key) => m_Dic.Remove(key);
        public bool Remove(KeyValuePair<TKey, TValue> item) => ((IDictionary<TKey, TValue>)m_Dic).Remove(item);
        public bool TryGetValue(TKey key, out TValue value) => m_Dic.TryGetValue(key, out value);

        IEnumerator IEnumerable.GetEnumerator() => ((IDictionary<TKey, TValue>)m_Dic).GetEnumerator();
        #endregion
        ...
        public void OnDeserialization(object sender) => m_Dic.OnDeserialization(sender);

        #endregion

    }
    #endregion
    #region SerializableDictionaryPropertyDrawer
#if UNITY_EDITOR
    public abstract class SerializableDictionaryPropertyDrawerBase : PropertyDrawer
    {
        ...
        protected static Dictionary<PropertyIdentity, ConflictState> s_conflictStateDict = new Dictionary<PropertyIdentity, ConflictState>();
        ...
        protected static float DrawKeyValueLine(SerializedProperty keyProperty, SerializedProperty valueProperty, Rect linePosition, int index)
        {
        ...
        protected static float DrawKeyValueLineSimple(SerializedProperty keyProperty, SerializedProperty valueProperty, string keyLabel, string valueLabel, Rect linePosition)
        {
        ...
        protected static float DrawKeyValueLineExpand(SerializedProperty keyProperty, SerializedProperty valueProperty, Rect linePosition)
        {
            float labelWidth = EditorGUIUtility.labelWidth;
        ...
        protected static float DrawKeyLine(SerializedProperty keyProperty, Rect linePosition, string keyLabel)
        {
            float keyPropertyHeight = EditorGUI.GetPropertyHeight(keyProperty);
            var keyPosition = linePosition;
            keyPosition.height = keyPropertyHeight;
        ...
        protected static bool CanPropertyBeExpanded(SerializedProperty property)
        {
        ...
        protected static void SaveProperty(SerializedProperty keyProperty, SerializedProperty valueProperty, int index, int otherIndex, ConflictState conflictState)
        {
        ...
        protected static ConflictState GetConflictState(SerializedProperty property)
        {
        ...
        protected static Dictionary<SerializedPropertyType, PropertyInfo> s_serializedPropertyValueAccessorsDict;
